...
(defn grow-cuboid
  "Add a cuboid branch of `length` to `mesh` using position and rotation in the 4x4 matrix `mat`."
  [length mat mesh]
...
(defn grow-cylinder
  "Add a cylinder branch of `length` to `mesh` using position and rotation in the 4x4 matrix `mat`."
  [thickness length mat mesh]
  (let [curr-mat (last mat)]
    [(conj (pop mat) (g/translate curr-mat 0 0 length))
     (let [tube (-> (circle thickness)
                    (g/center)
                    (g/extrude {:depth length}))
           ball (-> (sphere thickness) ;; for nicer joins
                    (g/center)
                    (g/as-mesh))]
       (g/into mesh (g/transform (g/into tube ball) curr-mat)))]))
